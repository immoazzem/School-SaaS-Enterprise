...
import type { ApiSchool, ApiUser } from '~/composables/useApi'
...
export const useAuthStore = defineStore('auth', () => {
  ...
  const selectedSchool = computed(() =>
    schools.value.find((school) => school.id === selectedSchoolId.value) ?? null,
  )

  const permissions = computed(() => selectedSchool.value?.permissions ?? [])
