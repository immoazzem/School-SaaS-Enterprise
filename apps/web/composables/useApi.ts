...
export type AuthSchool = ApiSchool
...
export function useApiBase() {
  ...
  return `${config.public.apiBase}/v1`
}
...
export function useApiFetch<T>(path: string, options: Parameters<typeof $fetch<T>>[1] = {}) {
  ...
  headers.set('Accept-Language', apiLocale.value)

  const activeToken = tokenState.value ?? accessToken.value

  if (activeToken)
    headers.set('Authorization', `Bearer ${activeToken}`)

  return $fetch<T>(`${useApiBase()}${normalizePath(path)}`, {
    ...options,
    headers,
  })
}
